...
Feature: NEWS Observation - High Clinical Risk
  As a HCA/Nurse
  ...
  Scenario: Triggered tasks shown to Nurse user after submitting observation
    ...
    And I submit a NEWS Observation for a patient with a high clinical risk
    And I confirm the calculated clinical risk to be High
  ...
  Scenario Outline: Triggered tasks shown in task list for Nurse
    Given I am logged in as a Nurse
    And I submit a NEWS Observation for a patient with a high clinical risk who has been in hospital for <stay duration> days
    And I confirm the calculated clinical risk to be High
    ...
    Then I should see the following new notifications for the patient
      ...
      | Immediately Inform Medical Team  |
      | Call An Ambulance 2222/9999      |
    And the next NEWS observation task for the patient is in <time to next NEWS> minutes
