...
class TestLowRiskHCAInformNurseAboutPatient(TransactionCase):
    """
    Test that the HCA and Nurse tasks are generated on a HCA submitting a low
    risk EWS observation
    """

    def setUp(self):
        super(TestLowRiskHCAInformNurseAboutPatient, self).setUp()
    ...
    def test_assess_patient_task_created(self):
        """
        Test that the Assess Patient task is generated at the same
        time
        """
        activity = self.test_utils.get_open_activities_for_patient(
