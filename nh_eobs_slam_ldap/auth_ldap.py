...
class SLaMUsersLDAP(osv.osv):
    """
    Override of the auth_ldap/user_ldap.py file to set the opt_referrals flag
    properly to work with SLaM's Active Directory server
    """
    ...
    def get_or_create_user(self, cr, uid, conf, login, ldap_entry,
        ...
        :rtype: int
        """
        user_id = super(SLaMUsersLDAP, self).get_or_create_user(
            cr, uid, conf, login, ldap_entry, context=context)
        user_obj = self.pool['res.users']
        user_obj.write(cr, SUPERUSER_ID, user_id, {'ldap_authenticated': True},
                       context=context)
        return user_id
